...
export const AddCard = (props) => {
    ...
    const [Account, Accounthandler, AccountregExpResult, AccountResetValue] = useForm("", ["^\\w+((-\\w+)|(\\.\\w+))*\\@[A-Za-z0-9]+((\\.|-)[A-Za-z0-9]+)*\\.[A-Za-z]+$"], ["請輸入正確E-mail格式"]); // 門市姓名欄位
    ...
    const [Location, Locationhandler, LocationregExpResult, LocationResetValue] = useSelector("", [(value) => ((value?.value ?? "").toString()?.length > 0)], ["請選擇所在門市"]); // 門市門市欄位
    ...
    //#region 新增用戶API
    const addAdminShops = useCallback(async (name, account, pass, phone, location, role) => {
                                ...
                                : (PhoneregExpResult ? alertService.warn(PhoneregExpResult, { autoClose: true })
                                    : (LocationregExpResult ? alertService.warn(LocationregExpResult, { autoClose: true })
                                        : (RoleregExpResult ? alertService.warn(RoleregExpResult, { autoClose: true })
                                            : AddAdminShopsExecute(Name, Account, Pass, Phone, Location, Role)
                                        )
                                    )
                                )
                            )
